...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        // Table Action DropDown Button Items - Part1
        ...
        // Table Action DropDown Button Component and Styles
        ...
        // Query with datatable library instead of models
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                ...
                // ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('warehouses')}.name as category_id, cost as cost, price as price, COALESCE(quantity, 0) as quantity, {$this->db->dbprefix('units')}.code as unit, '' as rack, alert_quantity", FALSE)
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('NEW_racks')}.name as category_id, cost as cost, price as price, COALESCE(quantity, 0) as quantity, {$this->db->dbprefix('units')}.code as unit, '' as rack, alert_quantity", FALSE)
                ->from('products')
                // ->join('categories', 'products.category_id=categories.id', 'left')
                // ->join('warehouses', 'products.category_id=warehouses.id', 'left')
                ->join('NEW_racks', 'products.category_id=NEW_racks.id', 'left')
                ->join('units', 'products.unit=units.id', 'left')
                ->join('brands', 'products.brand=brands.id', 'left')
                ->group_by("products.id");
        }
        ...
        ////////////////////////////////////////////////////////////////////////
        // Show/Hide data based on user category
        ...
        // Add the Actions column - Keywords in "" probably passing row info
        $this->datatables->add_column("Actions", $action, "productid, image, code, name");

        // Using the datatables library's method generate() to output data
    ...
    function add($id = NULL)
    {
        ...
        ********************************************************************* */
        ...
        if ($this->form_validation->run() == true) {
            ...
            /* *****************************************************************
              SPECIFIC LOGIC VALIDATION - PRODUCT TYPE STANDARD
            ***************************************************************** */
            ...
            /* *****************************************************************
              SPECIFIC LOGIC VALIDATION - PRODUCT TYPE SERVICE
            ***************************************************************** */
            ...
            /* *****************************************************************
              SPECIFIC LOGIC VALIDATION - PRODUCT TYPE COMBO
            ***************************************************************** */
            ...
            /* *****************************************************************
              SPECIFIC LOGIC VALIDATION - PRODUCT TYPE DIGITAL
              FILE UPLOAD
            ***************************************************************** */
        ...
        /* *********************************************************************
          RUNS FORM VALIDATION AGAIN AND ADDS PRODUCT
          IF EVERYTHING WENT OK, SUCCESS MESSAGE AND REDIRECT
          ELSE, ERROR MESSAGE AND RETURN TO FORM PAGE
        ********************************************************************* */
