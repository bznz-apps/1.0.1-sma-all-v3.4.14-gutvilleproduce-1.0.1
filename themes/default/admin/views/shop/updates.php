        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <?php
                        if ($shop_settings->purchase_code == 'purchase_code' || $shop_settings->envato_username == 'envato_username') {
                            echo admin_form_open('shop_settings/updates');
                        ?>
                            ...
                            if (!empty($updates->data->updates)) {
                                ...
                                foreach ($updates->data->updates as $update) {
                                    ...
                                    if ($c == 1 && $update->mversion != 315) {
                                        echo anchor('admin/shop_settings/install_update/' . substr($update->filename, 0, -4) . '/' . (!empty($update->mversion) ? $update->mversion : 0) . '/' . $update->version, '<i class="fa fa-download"></i> ' . lang('install'), 'class="btn btn-primary pull-right"') . ' ';
                                    }
